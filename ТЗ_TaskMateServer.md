...
| DEAL-002 | Мягкое удаление |
| DEAL-003 | Каждый салон изолирован (мультитенантность) |
...
| PROOF-002 | Максимальный размер файла: 200 МБ |
| PROOF-003 | Разрешённые форматы: JPG, PNG, GIF, WebP, MP4, MOV, PDF, ZIP |
| PROOF-004 | Валидация содержимого по magic bytes (не только по расширению) |
| PROOF-005 | Приватное хранилище: `storage/app/private/task_proofs/` |
| PROOF-006 | Доступ через подписанные URL (срок действия: 60 минут) |
| PROOF-007 | Удаление файлов при удалении ответа |
...
| ID | Описание |
|----|----------|
| CAL-001 | Получение календаря на год |
| CAL-002 | Получение праздников на год |
...
| CAL-007 | Приоритет: сначала салон, затем глобальные |
...
| AUDIT-004 | История конкретной записи |
| AUDIT-005 | Доступ только для owner |
...
| POST | /settings/shift-config | Обновление конфига смен |
| PUT | /settings/notification-config | Обновление конфига уведомлений |
| PUT | /settings/archive-config | Обновление конфига архивации |
| PUT | /settings/task-config | Обновление конфига задач |
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET | /calendar/{year} | Календарь на год |
| GET | /calendar/{year}/holidays | Праздники |
| GET | /calendar/check/{date} | Проверка даты |
| PUT | /calendar/{date} | Установка дня |
| DELETE | /calendar/{date} | Удаление переопределения |
| POST | /calendar/bulk | Массовые операции |

#### Аудит
| Метод | Путь | Описание |
|-------|------|----------|
| GET | /audit-logs | Логи |
| GET | /audit-logs/actors | Список акторов |
| GET | /audit-logs/{table}/{id} | История записи |
